...
use BingAds\Optimizer\GetBidOpportunitiesRequest;
...
try
{
    ...
    $opportunities = GetBidOpportunities($AccountId, $AdGroupId, $CampaignId, $proxy);
    
    if (empty($opportunities->Opportunity))
    {
        print "There are not any bid opportunities for the specified account.\n";
    }
    else
    {
        ...
        foreach ($opportunities as $opportunity)
        {
            ...
            if (null != $opportunity && sizeof($opportunityKeys) < 10000)
            {
...
function GetBidOpportunities($accountId, $adGroupId, $campaignId, $proxy)
{
    ...
    try
    {
        return $proxy->GetService()->GetBidOpportunities($request)->Opportunities;
    }
    catch (SoapFault $e)
    {
